...
def parseData( data, robot, verbose=False ):
    ...
    if frameType == ARNETWORKAL_FRAME_TYPE_ACK:
        ...
        if verbose:
            print ("ACKACK"+str( ord(data[frameSize-1])))
        data = data[frameSize:]
        return data
    ...
    if frameId == 0x7F:
        ...
        if commandProject == 0:
            ...
            if (commandClass, commandId) == (4,4):
                ...
                if verbose:
                    print ("Position"+str(lat)+str(lon)+str(alt))
            ...
            print ("UNKNOWN Project"+str( commandProject))
    elif frameId == 0x7E:
        commandProject, commandClass, commandId = struct.unpack("BBH",  data[7:7+4])
        if (commandProject, commandClass) == (0,3):
            ...
            if verbose:
                print ("Date:"+str( data[11:frameSize-1]))
        elif (commandProject, commandClass, commandId) == (0,5,5):
            if verbose:
                print ("Time:"+str( data[11:frameSize-1]))
            ...
            if verbose:
                print("WiFi Outdoor:"+str( struct.unpack("B", data[11:12])[0]))
            ...
            if commandId == 0:
                ...
                #print"Flying State"+str(( state, states[state])))
            elif commandId == 2:
                ...
                #print"ALERT State"+str( (state, states[state])))
            elif commandId == 3:
                ...
                #print"NavigateHomeStateChanged"+str(( state, states[state], reasons[reason])))
                robot.navigateHomeState = state
            else:
                #print"Unknown Piloting State"+str( commandId))
                printHex( data[:frameSize] )
            ...
            if commandId == 0:
                ...
                if verbose:
                    print("MaxAltitude:"+str( struct.unpack("fff", data[11:11+3*4])))
            elif commandId == 1:
                # ARCOMMANDS_ID_ARDRONE3_PILOTINGSETTINGSSTATE_CMD_MAXTILTCHANGED = 1,
                if verbose:
                    print("MaxTilt:"+str( struct.unpack("fff", data[11:11+3*4])))
            elif commandId == 2:
                # ARCOMMANDS_ID_ARDRONE3_PILOTINGSETTINGSSTATE_CMD_ABSOLUTCONTROLCHANGED,
                if verbose:
                    print("AbsoluteControl:"+str( struct.unpack("B", data[11:12])[0]))
            else:
                #print"Unknown Piloting Settings State"+str( commandId)
                printHex( data[:frameSize] )
            ...
            if verbose:
                ...
                #print"Picture State Changed:"+str(( state, massStorageId
            ...
            if verbose:
                ...
                #print"Video State Changed:"+str(( states[state], massStorageId
            ...
            if commandId == 4:
                ...
                if verbose:
                    print("Motor flights status"+str(( nbFlights, lastFlightDuration, totalFlightDuration)))
            elif commandId == 5:
                # ARCOMMANDS_ID_ARDRONE3_SETTINGSSTATE_CMD_MOTORERRORLASTERRORCHANGED = 5
                lastError = struct.unpack("I", data[11:11+4])[0]
                if verbose:
                    print("Motor last error"+str( lastError))
            else:
                if verbose:
                    #print"Settings state"+str( commandId,
                    printHex( data[:frameSize] )

        elif (commandProject, commandClass, commandId) == (1,20,5):
            # ARCOMMANDS_ID_ARDRONE3_CLASS_PICTURESETTINGSSTATE = 20,            
            if verbose:
                print("VIDEOAUTORECORDCHANGED"+str( struct.unpack("BB", data[11:11+2])))
            ...
            if verbose:
                print("Video Enabled State"+str(( state, states[state])))
            ...
            if verbose:
                print("Home changed"+str( struct.unpack("dd", data[11:11+16])))

        elif (commandProject, commandClass, commandId) == (1,24,2):
            # ARCOMMANDS_ID_ARDRONE3_CLASS_GPSSETTINGSSTATE = 24,
            # ARCOMMANDS_ID_ARDRONE3_GPSSETTINGSSTATE_CMD_GPSFIXSTATECHANGED = 2,
            if verbose:
                print("GPSFixStateChanged - fixed:"+str( struct.unpack("B", data[11:11+1])))

        elif (commandProject, commandClass, commandId) == (129,3,0):
            if verbose:
                print("GPSDebugState, numSat ="+str( struct.unpack("B", data[11:11+1])[0]))
