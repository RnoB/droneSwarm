...
class visionAnalyzer(PiRGBAnalysis):
    ...
    def analyze(self,frame):
        ...
        if not analyze:
            ...
            maskRB = np.subtract(frameC[:,:,2], frameC[:,:,0].astype(np.int16))*self.circle
            #maskRB = frameC[:,:,2].astype(np.int16)*self.circle
...
def bayerCapture():
    ...
    with picamera.PiCamera() as camera:
        ...
        with io.BytesIO() as stream:
            while  mode == 3:
                ...
                #frame = np.zeros(data.shape + (3,), dtype=data.dtype)
                #frame[1::2, 0::2, 0] = data[1::2, 0::2] # Red

                frame = data
                #frame[0::2, 0::2, 1] = data[0::2, 0::2] # Green
                #frame[1::2, 1::2, 1] = data[1::2, 1::2] # Green
                #frame[0::2, 1::2, 2] = data[0::2, 1::2] # Blue
                # At this point the image is available as stream.array
                #frame = stream.array#(stream.demosaic() >> 2).astype(np.uint8)
    ...
    #t0=time.time()
                ...
                if not analyze:
                    ...
                    if capture:
                        timestr = time.strftime("%Y%m%d-%H%M%S")
                        cv2.imwrite('/home/pi/Pictures/Blue'+timestr+'.tiff',frame)
                        capture = False
                else:
                    maskRB = np.subtract(frame[:,:,2], frame[:,:,0].astype(np.int16))*self.circle
                    #maskRB = frameC[:,:,2].astype(np.int16)*self.circle
                    maskRB=maskRB>self.thresholdRed
                    maskdRB = (np.roll(maskRB,1,axis=1) != np.roll(maskRB,-1,axis=1))
                    
                    frameD = cv2.bitwise_and(frame,frame,mask = maskRB.astype(np.uint8))
                    n = str(self.i).zfill(5)
                    self.duV = np.sum(self.VcoscosR[maskRB])
                    self.dudV = np.sum(self.VcoscosA[maskdRB])/2.0
                    self.dpV = np.sum(self.VcossinR[maskRB])
                    ...
                    if capture:
                        timestr = time.strftime("%Y%m%d-%H%M%S")
                        cv2.imwrite('/home/pi/Pictures/Blue'+timestr+'.jpg',frame)
                        cv2.imwrite('/home/pi/Pictures/Cut'+timestr+'.jpg',frameD)
                        cv2.imwrite('/home/pi/Pictures/Thres'+timestr+'.jpg',maskRB.astype(np.uint8)*255)
                        capture = False
